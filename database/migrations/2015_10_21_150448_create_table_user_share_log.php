...
class CreateTableUserShareLog extends Migration
{
    ...
    public function up()
    {
        Schema::create('user_share_log', function (Blueprint $table) {
            ...
            $table->string('share_water_count')->default('0')->comment('分享的亲水总值');
            $table->string('share_time')->default('0')->comment('分享的批次');
            $table->string('share_count')->default('0')->comment('分享的次数');
            $table->string('share_receive')->default('0')->comment('已经被领取的亲水值');
